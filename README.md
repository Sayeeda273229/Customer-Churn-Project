# Customer-Churn-Project
Customer-Churn-Prediction-Analysis
🧠 Customer Churn Prediction Analysis
Predicting telecom customer churn using Machine Learning to identify key drivers of customer retention.

📊 Project Overview
This project applies data analytics and machine learning techniques to predict whether a customer will leave a telecom service.
Includes data cleaning, feature engineering, EDA, and modeling with XGBoost for high predictive accuracy.
The goal is to help businesses understand which factors contribute most to churn and design better customer retention strategies.

🧩 Features
Comprehensive Exploratory Data Analysis (EDA)
Data preprocessing, feature encoding, and outlier handling
XGBoost model for high-accuracy churn prediction
Evaluation using confusion matrix, ROC-AUC, precision, and recall
Exported trained model (.joblib) for deployment

🧰 Tech Stack
Python, Pandas, NumPy, Seaborn, Scikit-learn, XGBoost, Matplotlib

🚀 How to Run
# Clone repository
git clone (https://github.com/Sayeeda273229/Customer-Churn-Project)
cd customer-churn-prediction-analysis

# Create and activate a virtual environment
python -m venv venv
source venv/bin/activate    # For Mac/Linux
venv\Scripts\activate       # For Windows

# Install dependencies
pip install -r requirements.txt

# Run the notebook
jupyter notebook notebook/customer_churn.ipynb


# Clone repository
git clone https://github.com/yourusername/customer-churn-prediction-analysis.git
